...
function getCamMicStatus() {
    try {
        if (process.platform === "win32") {
            return execSync('powershell -Command "Get-PnpDevice -Class Camera"').includes("Enabled") ? "Camera/Microphone in use" : "Not in use";
        } else if (process.platform === "darwin" || process.platform === "linux") {
            return execSync("lsof | grep /dev/video", { encoding: "utf-8" }).split("\n").length > 1 ? "Camera/Microphone in use" : "Not in use";
        } else if (process.platform === "android") {
            return execSync("dumpsys media.camera | grep 'Client'", { encoding: "utf-8" }).includes("Client") ? "Camera/Microphone in use" : "Not in use";
        }
    } catch {
        return "Unknown";
    }
